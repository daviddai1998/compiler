...
int main( int argc, char **argv ) {
  ...
  if (argc != 2) {
    ...
    fprintf(stderr,"Valid form is '%s log_file'\n",
	    argv[0]);
    exit(-1);
  }
  else {
  ...
  while( b_len != (size_t) -1) {
	...
	while(strncmp(data_line,"ERROR:",6) == 0) {
	  ...
	  FixCRLF(data_line);
        ...
        while(strlen(data_line) == 0) {
	  d_len = (int) getline(&data_line,&d_len,stdin);
	  data_line[d_len-1] = '\0';
	  FixCRLF(data_line);
	}
        while(strncmp(data_line,"FATAL:",6) == 0) {
          fatal = 1;
	  d_len = (int) getline(&data_line,&d_len,stdin);
	  data_line[d_len-1] = '\0';
	  FixCRLF(data_line);
	...
	if (strlen(fname) > 5)
	  fprintf(stderr,"Program '%s'\tcorrectly executed %d cycles\n",
		  fname,cycles);
	else
	  fprintf(stderr,"Program '%s'\t\tcorrectly executed %d cycles\n",
		  fname,cycles);
	  ...
	  fprintf(stderr,
                  "The scheduler found a FATAL error in the input file.\n");
	  fprintf(stderr,"See the log file; search for 'FATAL'.\n\n");
	  fatal = 0;
      }
      else 
	fprintf(stderr,"Some kind of error; fatal is %d.\n",fatal);
    }
  }
  return 0;
}
