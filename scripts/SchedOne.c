...
int main( int argc, char **argv ) {
  ...
  strcpy(temp_file,"~/SchedScriptTemporaryFile.i");

  if (argc != 4) {
    fprintf(stderr,"Invalid command line input.\n");
    fprintf(stderr,"Valid form is '%s iloc_file sched_path  sim_path'\n",
	    argv[0]);
    exit(-1);
  }
  else {
    ...
    if (flag!= 0) {
      fprintf(stderr,"Invalid path to scheduler '%s' (%d).\n",
	      argv[2],flag);
      exit(-1);
    }
    flag = stat(argv[3],&s_buf);
    if (flag!= 0) {
  ...
  if (found_input && found_output) {
    ...
    /* to change interlock levels, change the flag in the following stmt */
    sprintf(command_line,"%s -s 1 %s <%s\n",argv[3],input,temp_file);
    if (DEBUG)
      fprintf(stdout,"%s",command_line);
    else
      system(command_line);
